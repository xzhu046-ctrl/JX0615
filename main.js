...
const APP_BUILD_ID = '2026-05-13T17:39:47Z';
...
function collectShellAvatarDomSources(doc, selectors){
  var out = [];
  var seen = [];
  function pushSource(node){
    if(!node || seen.indexOf(node) >= 0) return;
    seen.push(node);
    var src = '';
    try{ src = String((node.getAttribute && (node.getAttribute('data-avatar-src') || node.getAttribute('src'))) || node.src || '').trim(); }catch(err){}
    src = normalizeShellAssetSrc(src);
    if(isRenderableShellAvatarSrc(src) && out.indexOf(src) === -1) out.push(src);
  }
  try{
    (selectors || []).forEach(function(selector){
      try{
        Array.prototype.slice.call(doc.querySelectorAll(selector)).forEach(pushSource);
      }catch(selErr){}
    });
    Array.prototype.slice.call(doc.querySelectorAll('img')).forEach(function(img){
      ...
      if(/avatar|头像|hdr|char|user|bond|wgt|offline-story|polaroid/i.test(text)) pushSource(img);
    });
  }catch(err){}
  return out.slice(0, 40);
}

function cleanupAvatarDebugWindows(){
  if(!document || !document.querySelectorAll) return;
  try{
    ['avatar-debug-style', 'avatar-debug-home-window', 'avatar-debug-app-window'].forEach(function(id){
      var el = document.getElementById(id);
      if(el && el.parentNode) el.parentNode.removeChild(el);
    });
    Array.prototype.slice.call(document.querySelectorAll('.avatar-debug-window')).forEach(function(el){
      if(el && el.parentNode) el.parentNode.removeChild(el);
    });
  }catch(err){}
}
...
function handleShellBondAvatarError(img, role){
  try{
    if(!img) return;
    var base = img.closest && img.closest('.bond-avatar-base');
    var target = base && base.parentNode ? base.parentNode : null;
    var failedSrc = normalizeShellAssetSrc(img.getAttribute('src') || img.src || '');
    var safeRole = String(role || '') === 'user' ? 'user' : 'char';
    var fallback = safeRole === 'user' ? '你' : 'C';
    if(base) base.textContent = fallback;
    if(!target || safeRole !== 'char') return;
    ...
    loadCharacterAvatarForShell(charId).then(function(nextSrc){
      nextSrc = normalizeShellAssetSrc(nextSrc || '');
      if(!isRenderableShellAvatarSrc(nextSrc) || nextSrc === failedSrc) return;
      applyBondAvatarContent('char', nextSrc, fallback, charId);
    }).catch(function(){});
  }catch(err){}
}
